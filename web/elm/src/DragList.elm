...
append item ({ items } as model) =
    case items of
        ( left, [] ) ->
            { model | items = ( left ++ [ Item (List.length left) item ], [] ) }

        ( _, _ ) ->
            Debug.log "bad append" model
    ...
    case msg of
        ...
        Remove i ->
            let
                ...
                left' =
                    left |> List.filter (\{ index } -> index /= i)

                right' =
                    right |> List.filter (\{ index } -> index /= i)
            in
                { model | items = ( left', right' ) } ! []

        Value i message ->
            let
                applyMsg ({ index, value } as item) =
                    if index /= i then
                        item ! []
                    else
                        let
                            ( v, cmd ) =
                                struct.update message value
                        in
                            ( Item i v, Cmd.map (Value i) cmd )

                lefts =
                    List.map applyMsg (fst items)

                rights =
                    List.map applyMsg (snd items)

                items' =
                    ( List.map fst lefts, List.map fst rights )

                cmds =
                    List.foldl (::) [] (lefts ++ rights |> List.map snd)
            in
                { model | items = items' } ! cmds
...
view { drag, items, struct } =
    let
        ...
        list =
            (List.map toElement left)
                ++ (case drag of
                        Nothing ->
                            List.map toElement right
